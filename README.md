...
如果用户提到母目录、旧工作目录、历史优秀结果或同一作业的已完成 `.mdj`/PNG，Skill 必须优先把它作为质量基线。对同一项目，优先复制/适配高质量 `.mdj`，再做局部修复和验证；不要从零重画出一个明显更差的工程。manifest 需要记录 `baselineUsed`、基线 `.mdj`、基线 PNG 目录和对齐状态。

推荐优先复用的本地资产类型：旧的高质量 `.mdj`、已导出的 `图形导出/` PNG、父目录 `mcp/apply-course-style.mjs` / `mcp/fix-layout.mjs` / `mcp/final-clean-class-diagram.mjs` 等人工式布局脚本，以及 `.lbssb/` 现场记录和课堂建模规范总结。
...
最新 PNG 必须重新复核。只要 native PNG 在 `visual-review.json` 之后被重新导出，旧复核结果立即失效，manifest 不能继续标 `Verified`。清晰重绘 PNG 可以作为 `semantic-consistent` 辅助交付，但不能证明原生 `.mdj` 图布局已经优化。

可接受的高质量原生修图方式是“课程风格修图”：读取真实 StarUML View，按图名设置最终 bounds，按关系设置 edge points，逐类图导出复核。不可接受的方式是只依赖 Mermaid 导入、全局 auto-layout 或一次性批量生成。
...
- 顺序图消息没有可见编号。
- 用例图 actor 直接连接过多细粒度用例，没有入口用例或模块区。
- 类图在生成脚本里硬编码重建成员，覆盖源 `.mdj` 的英文变量/方法。
